...
Token = 'OTQ3OTYzMzMyMDcyMTkwMDEy.G-Bke-.3klZIrIorzvlBNR5OGFi0wzoVge59xQkf39VwE'
bot_version_info = "Running on Version 1.0"

logging.basicConfig(filename='Logging.log', encoding='utf-8', level=logging.WARNING)


def active_file_check(member_id):
    with open('active_member.json', 'r') as f:
...
async def on_message(message):
    ...
    # Informations saved as a Variable
    date_time = datetime.now().strftime("%m/%d/%Y, %H:%M")
    title = f"{message.author.id}"
    json_object = {"last_message": f"{date_time}", "name": f"{message.author.name}"}

    # Writing the Activity
    dump(title, json_object, "active_member.json")
...
async def on_raw_reaction_add(payload):
    ...
    if payload.channel_id == 927015318424002680:
        ...
        if payload.message_id == c_channel_msg_id: #Main Message
            if payload.emoji.name == '👨‍👩‍👦‍👦':

                #Answering
                TextChannel = bot.get_channel(payload.channel_id)
                message = TextChannel.get_partial_message(payload.message_id)
                channel = bot.get_channel(payload.channel_id)
                await channel.send(content="Für wie viele Personen soll in dem Channel Platz sein? Reagiere... ")
            ...
            if not payload.user_id == 947963332072190012:
                ...
                channel = bot.get_channel(payload.channel_id)
                main_message = await channel.fetch_message(c_channel_msg_id)

                reaction_users = main_message.reactions[0]
                user_info = [user async for user in reaction_users.users()]

                #All users that have reacted right now in a list
                user_ids = []
                ...
                if payload.user_id in user_ids:
                    ...
                    #deleting the reaction on the main message
                    member = guild.get_member(payload.user_id)
                    channel = bot.get_channel(payload.channel_id)
                    message = channel.get_partial_message(c_channel_msg_id)

                    await message.remove_reaction(emoji='👨‍👩‍👦‍👦', member=member)
                    ...
                    #Deleting the message of the bot
